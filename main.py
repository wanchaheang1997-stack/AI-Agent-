...
# កំណត់ Logging ដើម្បីមើល Error ក្នុង Render ឱ្យច្បាស់
...
def check_luxalgo_signal():
    try:
        ...
        if last_c > upper[-1] and prev_c <= upper[-2]: return "🚀 BUY", last_c
        if last_c < lower[-1] and prev_c >= lower[-2]: return "⚡️ SELL", last_c
        return None, last_c
...
async def job_signal(context: ContextTypes.DEFAULT_TYPE):
    sig, price = check_luxalgo_signal()
    if sig:
...
async def main():
    ...
    # កំណត់ឱ្យ Bot កុំព្យាយាមទាញយកសារចាស់ៗពេក ដើម្បីកាត់បន្ថយ Conflict
    ...
    scheduler.add_job(job_signal, 'interval', minutes=1, args=[app])
    scheduler.start()

    async with app:
        await app.initialize()
        await app.start()
        # កែត្រង់ចំណុចនេះ ដើម្បីឱ្យវាទាញយកតែសារថ្មីៗ
